...
public class ImgCompress implements Handler.Callback {
  ...
  private static final String DEFAULT_DISK_CACHE_DIR = "fy_disk_cache";
  ...
  private File getImageCacheFile(Context context, String suffix) {
    ...
    String cacheBuilder =
        mTargetDir + "/" + System.currentTimeMillis() + (int) (Math.random() * 1000) + (
          ...
          @Override public void run() {
            try {
              ...
              File result = Checker.isNeedCompress(mLeastCompressSize, path) ? new Engine(path,
                  getImageCacheFile(context, Checker.checkSuffix(path))).compress()
                  : new File(path);
